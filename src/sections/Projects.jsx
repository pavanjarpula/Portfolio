...
/*
  ...
  card  = 30vw wide
  gap   = 1.5vw
  slot  = 31.5vw  (card + gap)

  To center cards [A, B] in a 100vw viewport:
    x = (100vw - 30vw - 1.5vw - 30vw) / 2  =  (100 - 61.5) / 2  =  19.25vw

  To center cards [C, D] (positions 2,3) instead:
    x = 19.25vw - 2 * 31.5vw  =  19.25 - 63  =  -43.75vw

  ROW 1: [01][02][03][04][05][06][07][08]
    scroll=0 → x = +19.25vw  (01,02 centred)
    scroll=1 → x = -43.75vw  (07,08 centred)   moves LEFT ←
*/
...
function usePixelOffsets() {
  const calc = () => {
    ...
    return {
      start: ((100 - 30 - 1.5 - 30) / 2 / 100) * vw,   // 19.25vw in px
      end:   ((19.25 - 63) / 100) * vw,                   // -43.75vw in px
    };
  };
...
function ProjectCard({ project }) {
  return (
    ...
    >
      <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '3px', background: project.color, opacity: 0.7 }} />
      <div style={{ position: 'absolute', top: '-40px', right: '-40px', width: '140px', height: '140px', borderRadius: '50%', background: `radial-gradient(circle, ${project.color}0a 0%, transparent 70%)`, pointerEvents: 'none' }} />

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
        <div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: project.color, letterSpacing: '0.15em', marginBottom: '4px' }}>{project.id} / {project.year}</div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(15px,1.5vw,20px)', fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em', lineHeight: 1.2, margin: 0 }}>{project.title}</h3>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '2px' }}>{project.subtitle}</div>
        </div>
        <a href={project.github} target="_blank" rel="noopener noreferrer"
          style={{ width: '28px', height: '28px', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '12px', textDecoration: 'none', transition: 'all 0.3s', flexShrink: 0 }}
      ...
      <p style={{ fontSize: '11px', lineHeight: 1.65, color: 'var(--text-muted)', marginBottom: '14px', flexGrow: 1 }}>{project.description}</p>

      <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '8px' }}>
        {project.metrics.map((m, i) => (
          <div key={i} style={{ padding: '2px 8px', background: project.color + '10', border: `1px solid ${project.color}25`, borderRadius: '100px', fontSize: '9px', fontWeight: 600, color: project.color }}>{m}</div>
        ))}
      </div>

      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        {project.tags.map((tag, i) => (
          <span key={i} style={{ padding: '2px 7px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '3px', fontSize: '9px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{tag}</span>
        ))}
      </div>
    </motion.div>
  );
  ...
  return (
    <section id="projects" ref={sectionRef} style={{ padding: '80px 0', position: 'relative' }}>
      ...
      <div style={{ overflow: 'hidden', position: 'relative' }}>
        ...
        {/* ROW 1 */}
        <motion.div style={{ display: 'flex', gap: '1.5vw', x: xRow1, willChange: 'transform', marginBottom: '16px' }}>
          {projects.map((p, i) => <ProjectCard key={'r1-' + i} project={p} />)}
        </motion.div>

        {/* ROW 2 */}
        <motion.div style={{ display: 'flex', gap: '1.5vw', x: xRow2, willChange: 'transform', marginBottom: '16px' }}>
          {row2.map((p, i) => <ProjectCard key={'r2-' + i} project={p} />)}
        </motion.div>

        {/* ROW 3 */}
        <motion.div style={{ display: 'flex', gap: '1.5vw', x: xRow3, willChange: 'transform', marginBottom: '16px' }}>
          {row3.map((p, i) => <ProjectCard key={'r3-' + i} project={p} />)}
        </motion.div>

        {/* ROW 4 */}
        <motion.div style={{ display: 'flex', gap: '1.5vw', x: xRow4, willChange: 'transform' }}>
      ...
      <div className="container">
        <div style={{ marginTop: '32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {projects.map((_, i) => (
                <motion.div key={i} style={{
                  height: '2px', width: '20px', borderRadius: '2px',
          ...
          <a href="https://github.com/pavanjarpula" target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', padding: '10px 24px', background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.2)', borderRadius: '4px', color: 'var(--accent)', textDecoration: 'none', fontSize: '12px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', transition: 'all 0.3s' }}
